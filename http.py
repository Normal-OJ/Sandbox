...
def submit():
    if request.method == 'POST':
        ...
        languageId=request.values['languageId'] #0:C,1:C++,2:python3
        language_type=''#transfer languageID to language class
        if languageId==0:
            language_type="c"
        elif languageId==1:
            language_type="cpp"
        ...
        submissionId=request.values['submissionId']
        ...
        archize_src=zipfile.ZipFile('C:/test/'+submissionId+'/src.zip', 'r')
        archize_src.extractall('C:/test/'+submissionId)
        archize_src.close()
        
        archize_testcase=zipfile.ZipFile('C:/test/'+submissionId+'/testcase.zip', 'r')
        archize_testcase.extractall('C:/test/'+submissionId)
        archize_testcase.close()
           
        if len(os.listdir('C:/test/'+submissionId+'/src/'))==0:#len(['1','2','3'])
            return "under src does not have any file-400",400
        else:
            for files in walk('C:/test/'+submissionId+'/src/'):
                print (files)
                ...
                if file_name=="main":
                    ...
                    if which_file==language_type:
                        ...
                        if len(folder_in_0)==len(folder_out_0) and len(folder_in_1)==len(folder_out_1):
                            if (os.path.isfile('C:/test/'+submissionId+'/meta.json')):
                                # read file
                                with open('C:/test/'+submissionId+'/meta.json', 'r') as myfile:
                                    data=myfile.read()
                                # parse file
                                obj = json.loads(data)
