...
def test_normal_submission(docker_dispatcher: Dispatcher,
    ...
    for _id, prob in _ids:
        assert docker_dispatcher.handle(
            _id, submission_generator.problem[prob]['meta']['lang']) is True


def test_duplicated_submission(docker_dispatcher: Dispatcher,
                               submission_generator):
    import random
    docker_dispatcher.start()
    ...
    try:
        docker_dispatcher.handle(
            _id, submission_generator.problem[prob]['meta']['lang'])
